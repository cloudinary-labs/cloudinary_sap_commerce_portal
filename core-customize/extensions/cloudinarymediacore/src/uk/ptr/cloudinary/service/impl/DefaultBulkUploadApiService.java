...
public class DefaultBulkUploadApiService implements BulkUploadApiService {
    ...
    public void bulkAssetUpload(BulkUploadRequestData bulkUploadRequestData, String baseSiteId) {
        ...
        bulkUploadRequestData.getProductMediaAssest().forEach(bulkUpload -> {
            ...
            if (stagedProduct != null) {
                if (org.apache.commons.collections.CollectionUtils.isEmpty(stagedProduct.getGalleryImages())) {
                    ...
                    if (!CollectionUtils.isEmpty(mediaContainersData)) {
                        ...
                        mediaContainersData.forEach(md -> {
        ...
        if (!CollectionUtils.isEmpty(catalogVersionModels)) {
            catalogVersionModels.forEach(c -> {
                // Skip execution if catalog is a ClassificationCatalogModel
                if (!(c.getCatalog() instanceof ClassificationSystemModel)) {
                    try {
                        CatalogVersionModel onlineVersion = catalogVersionService.getCatalogVersion(
                                c.getCatalog().getId(), CloudinarymediacoreConstants.VERSION_ONLINE
                        );
                        catalogSynchronizationService.synchronizeFullyInBackground(c, onlineVersion);
                    } catch (Exception e) {
                        LOG.warn("Online version not found for catalog: " + c.getCatalog().getId() + ". Skipping sync.");
                    }
                }
            });
        }
